...
class DownloaderApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        # --- ウィジェットの作成 ---
        # ユーザーID
        ttk.Label(main_frame, text="ユーザーID (@不要):").grid(row=0,
        ...
        # Cookieファイル
        ttk.Label(main_frame, text="Cookieファイル:").grid(
            row=1, column=0, sticky=tk.W, pady=2)
        self.cookie_path_var = tk.StringVar(
            value=os.path.abspath('cookies.txt'))
        ...
        ttk.Entry(cookie_frame, textvariable=self.cookie_path_var,
                  state='readonly').grid(row=0, column=0, sticky=tk.EW)
        ttk.Button(cookie_frame, text="選択...", command=self.select_cookie_file).grid(
            row=0, column=1, padx=(5, 0))

        # 保存先フォルダ
        ttk.Label(main_frame, text="保存先フォルダ:").grid(
            row=2, column=0, sticky=tk.W, pady=2)
        self.output_dir_var = tk.StringVar(
            value=os.path.abspath('./downloads'))
        output_frame = ttk.Frame(main_frame)
        output_frame.grid(row=2, column=1, sticky=tk.EW, pady=2)
        output_frame.columnconfigure(0, weight=1)
        ttk.Entry(output_frame, textvariable=self.output_dir_var,
                  state='readonly').grid(row=0, column=0, sticky=tk.EW)
        ttk.Button(output_frame, text="選択...", command=self.select_output_directory).grid(
            row=0, column=1, padx=(5, 0))
        ...
        # 待機時間
        ttk.Label(main_frame, text="設定:").grid(
        ...
        ttk.Spinbox(settings_frame, from_=0, to=60, textvariable=self.sleep_var, width=5).pack(
    ...
    def select_cookie_file(self):
        path = filedialog.askopenfilename(title="cookies.txtを選択", filetypes=[
    ...
    def run_download(self):
        ...
        try:
            ...
            self.after(0, self.log_message, "-" * 100 + "\n")
